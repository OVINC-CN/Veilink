...
main 分支会发布 `ghcr.io/ovinc-cn/veilink` 和 `ghcr.io/ovinc-cn/veilink-turn` 两套多架构镜像，同时生成 `latest`、不可变 `sha-*` 标签以及 SBOM/Provenance。生产环境应通过 `VEILINK_IMAGE` 和 `VEILINK_TURN_IMAGE` 固定到 `sha-*` 标签。
